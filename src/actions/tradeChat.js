  ...
  function createTradeChat(tradeId) {
    return (dispatch) => {
      ...
      return api
        ...
        .then((tradeChatId) => {
          dispatch(
            createAction(tradeChatActionTypes.TRADE_CHAT_CREATE_TRADE_CHAT_SUCCESS, { tradeChatId })
          );
          return tradeChatId;
        })
        .catch((error) => {
          dispatch(
            createAction(tradeChatActionTypes.TRADE_CHAT_CREATE_TRADE_CHAT_FAILURE, { error })
          );
          throw error;
        });
    };
  ...
  function fetchTradeChats(userId, authToken) {
    return (dispatch) => {
      ...
      return api
        ...
        .then((tradeChats) => {
          dispatch(
            createAction(tradeChatActionTypes.TRADE_CHAT_FETCH_TRADE_CHATS_SUCCESS, { tradeChats })
          );
          return tradeChats;
        })
        .catch((error) => {
          dispatch(
            createAction(tradeChatActionTypes.TRADE_CHAT_FETCH_TRADE_CHATS_FAILURE, { error })
          );
          throw error;
        });
    };
