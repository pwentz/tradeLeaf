...
describe('chat socket actions', () => {
  ...
  context('initiateConversation', () => {
    it('sends a coded message to the chat socket', () => {
      const tradeChatId = 123;
      const recipientId = 15;

      mockDispatch.dispatch(actions.initiateConversation({ tradeChatId, recipientId }))

      expect(mockDispatch.actions).to.deep.equal([
        { type: chatSocketActionTypes.CHAT_SOCKET_INITIATE_CONVERSATION, tradeChatId, recipientId },
        { type: chatSocketActionTypes.CHAT_SOCKET_SEND_MESSAGE }
      ])

      expect(mockApi.sentMessage).to.equal('^{{ 123 - 15 }}^')
    })
  })
  ...
  context('send', () => {
    it('creates a message, then sends it to the chat server', (done) => {
      ...
      mockDispatch.dispatch(actions.send({ tradeChatId, senderId, content, token }))
        .then(() => {
          expect(mockDispatch.actions).to.deep.equal([
            { type: messageActionTypes.MESSAGE_CREATE_MESSAGE, tradeChatId, senderId, content },
            { type: messageActionTypes.MESSSAGE_CREATE_MESSAGE_SUCCESS, messageId: 3 },
            { type: chatSocketActionTypes.CHAT_SOCKET_SEND_MESSAGE }
          ])

          expect(mockApi.sentMessage).to.equal(`#{{ ${tradeChatId} }}#: ${content}`)
          done()
        })
        .catch(done)
    })
